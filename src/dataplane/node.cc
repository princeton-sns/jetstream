...
Node::handle_alter (const AlterTopo& topo)
{
  ...
  for (int i=0; i < topo.edges_size(); ++i) {
    ...
    else if (edge.has_dest_addr()) {   
      // remote network operator
      const NodeID &nodeId = edge.dest_addr();
      shared_ptr<TupleReceiver> outgoing
	(new OutgoingConnAdaptor(*connMgr, nodeId.address(), nodeId.portno()) );
      srcOperator->set_dest(outgoing);
    } 
    else {
