...
void TupleBatch::flush()
{
  ...
  msec_t now = get_msec();
  LOG_IF(INFO, (now+10)-start_time >2000)  << "Finished flush in "<< now-start_time <<" of "
    << tpi_store.size() <<" elements with "
    << num_flushes << " flushes. Rate = "<<  (now-start_time >0 ? (double)tpi_store.size()/(now-start_time):0) << " tuples/ms";
  /*
