...
def validate_TRound(in_schema, cfg):
  ...
  if t != "I":
    raise SchemaError("rounding operator requires that field %d be an int, instead was %s" % (fld_offset,t))
  return in_schema
...
SCHEMAS[OpType.RAND_SOURCE] = lambda schema,cfg: [('S','state')]
