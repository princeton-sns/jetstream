...
def main():
  ...
  parser.add_option("-m", "--min_bwidth", dest="mbwidth", help="minimum bandwidth allocation (Kbytes/sec) for file-based simulation [default = %d]" % (MIN_BWIDTH_DEFAULT), default=MIN_BWIDTH_DEFAULT)
...
#    errorExit("Error: Must specify a trace file or a fixed bandwidth.")
  ...
  bwidthVals = []
  if options.fname != "":
    bwidthVals = read_trace(options.fname, interval)
  else:    # Use the specified (fixed) bandwidth
    assert fbwidth > 0
